	...
	?>
		...
		<?php if ($featvideo) { ?>
			<div class="page-hero | wp-block-cover bg-white min-h-min  |  print:py-8">
				...
				<video playsinline autoplay muted loop poster="<?php echo $featimg[0]; ?>"
					class="wp-block-cover__video-background print:hidden" data-object-fit="cover">
				...
				<div class="wp-block-cover__inner-container p-2 container flex flex-col gap-0  |  lg:p-4">
					...
					<div class="text-neutral-900 flex flex-col justify-center space-y-6  |  md:min-h-hero print:min-h-min">
						<h1
							class="text-neutral-900 leading-none tracking-light text-pretty shadow-neutral-50 drop-shadow-lg  |  lg:print:text-xl! print:text-black dark:text-neutral-100">
							<?php echo $page_title; ?>
						</h1>

						<h2
							class="leading-none text-pretty text-neutral-700! shadow-neutral-50 drop-shadow-lg  |  md:max-w-5xl lg:print:text-base! print:text-black! dark:text-atlantis-200!">
							<?php echo $brand_message; ?></h2>

						<?php if ((!empty($hero_cta1_text)) && (!empty($hero_cta1_url))) : ?>
							<div class="wp-block-buttons is-layout-flex wp-block-buttons-is-layout-flex">
								<div class="inline-block m-0">
									<a class="border-2 inline-flex items-center justify-center px-5 py-3 font-head font-semibold no-underline rounded-lg text-neutral-100 !bg-brand-red-dark border-brand-red-dark shadow-md shadow-neutral-950 hover:border-white hover:text-white"
										href="<?php echo $hero_cta1_url; ?>">
										<?php echo $hero_cta1_text; ?>
									</a>
								</div>
								<?php if ((!empty($hero_cta2_text)) && (!empty($hero_cta2_url))) { ?>
									<div class="inline-block m-0">
										<a class="border-2 inline-flex items-center justify-center px-5 py-3 font-head font-semibold no-underline rounded-lg bg-transparent border-neutral-200 text-neutral-200 shadow-md shadow-neutral-950 hover:bg-transparent hover:border-brand-blue-pale hover:text-brand-blue-pale"
											href="<?php echo $hero_cta2_url; ?>">
		...
		<section
			class="full-bleed ll-equal-vert-padding not-prose bg-atlantis-400 text-atlantis-950 break-inside-avoid  |  print:bg-transparent">
			<div class="px-2 container  |  lg:px-4">
				<div class="flex flex-col items-start gap-4  |  sm:flex-row sm:items-center md:justify-between lg:gap-8">
					...
					<div class="w-full  |  md:max-w-fit print:hidden">
						<div class="wp-block-button"><a class="wp-block-button__link wp-element-button" href="/contact/">Contact
								Us</a></div>
					</div>
				</div>
			</div>
